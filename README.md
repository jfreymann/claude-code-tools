...
This repository provides **global tools** (agents, commands, skills) that work across all your projects. For each project, you'll use the `/bootstrap` command to generate **project-specific** documentation and context.

**Important**: The template files in this repository are **reference material only**—they show what the final output looks like. The `/bootstrap` command generates files directly based on its instructions; it doesn't read from template files.

### Two-Step Setup

| Command | What it does | When to run |
|---------|-------------|-------------|
| `claude init` | Creates `.claude/` directory, settings, project config | **Optional** - Claude Code does this automatically when you first use it in a project |
| `/bootstrap` | Creates `claude.md`, `docs/` directory, and project-specific `/sync` and `/context` commands | **Run once** for new projects or existing projects without this structure |
...
2. **Claude Code initializes automatically** (or run `claude init` if needed)
...
3. **Run the bootstrap command**
   ...
   /bootstrap
   ```
   - Asks about your project (name, stack, purpose)
   - Generates `claude.md` (main project context)
   - Creates `docs/` with architecture, decisions, troubleshooting, changelog, roadmap, patterns
   - Creates project-specific commands: `/sync` and `/context`
   - Initializes git integration if applicable

4. **Start working**
   - `claude.md` loads automatically at session start
   - Use global agents (like `@typescript-expert`, `@rails-expert`)
   - Use global commands (like `/create-plan`, `/debug`)
   - Run `/sync` at the end of each session to capture changes

### What Gets Created

**Global (this repository - shared across all projects)**:
- Agents: Domain experts like `bash-expert`, `typescript-expert`, `rails-expert`
- Commands: Workflows like `/create-plan`, `/debug`, `/whats-next`
- Skills: Knowledge frameworks like `create-agent-skills`, `debug-like-expert`

**Project-specific (generated by `/bootstrap`)**:
- `claude.md` - Project context and rules
- `docs/` - Architecture, decisions, troubleshooting, changelog, roadmap, patterns
- `.claude/commands/sync.md` - Documentation sync command
- `.claude/commands/context.md` - Context loader command

### Key Principle

**Don't duplicate global tools into projects.** The global tools in `~/.claude/` work across all your projects. Only project-specific documentation and context belong in each project's directory.
